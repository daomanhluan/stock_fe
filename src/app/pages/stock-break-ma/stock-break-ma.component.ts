...
@Component({
  selector: 'stock-break-ma',
  templateUrl: './stock-break-ma.component.html',
  styleUrls: ['./stock-break-ma.component.scss'],
})
export class StockBreakMAComponent implements OnInit {
  ...
  loadData(req: FindStockRequest){
    this.stockService.findStockBreakMA(req).subscribe((res) => {
      ...
      this.stockHistories.forEach(e=>e.volume_str = e.volume.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ","));
    });
  }
